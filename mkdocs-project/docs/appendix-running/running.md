...
| `--disabled-datastores=`<br>`datastore1,datastore2,...` | Disable the indicated datastores, used to optimize performance, especially for batch processing.  The datastore configuration file will be superseded by the command line parameter value. Specify multiple datastores separated with commas.  Use a glob-style regular expression with `*` to match a pattern.  Datastores are disabled after the `--enable-datastores` parameter is handled. | All enabled datastores in datastore configuration files are enabled. |
| `--enabled-datastores=`<br>`datastore1,datastore2,...` | Enable the indicated datastores, used to optimize performance, especially for batch processing.  The datastore configuration file will be superseded by the command line parameter value. Specify multiple datastores separated with commas.  Use a glob-style regular expression with `*` to match a pattern.  Datastores are enabled before the `--disable-datastores` parameter is handled. | All enabled datastores in datastore configuration files are enabled. |
