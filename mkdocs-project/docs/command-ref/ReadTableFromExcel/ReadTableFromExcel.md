...
**<p style="text-align: center;">
![ReadTableFromExcel example worksheet](ReadTableFromExcel_SheetComments.png)
</p>**
...
|`ColumnIncludeFilters`|Indicate column names (as assigned by `ExcelColumnNames`) and pattern to use to include rows.  The format of the parameter is:<br>`ColumnName1:Pattern1,`<br>`ColumnName2:Pattern2,...`<br>where patterns can contain `*` to match a substring.  Any match will cause the row to be included (the patterns are OR'ed).|Include all rows.|
|`ColumnExcludeFilters`|Indicate column names (as assigned by `ExcelColumnNames`) and pattern to use to exclude rows.  For example, exclude rows with blanks in columns.  The format of the parameter is:<br>`ColumnName1:Pattern1,`<br>`ColumnName2:Pattern2,...`<br>where patterns can contain `*` to match a substring.  Any match will cause the row to be excluded (the patterns are OR'ed).|Include all rows.|
