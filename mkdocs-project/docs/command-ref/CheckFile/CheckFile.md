...
| |`SearchPattern`| A string to match if `Statistic` `=PatternMatchLineCount`.  Use `*` for wildcard to match a substring, otherwise the line must exactly match.  Multiple patterns can be specified separated by commas or can be or'ed using syntax `*pattern1*|*pattern2*`. Can use `${Property}` notation. | |
