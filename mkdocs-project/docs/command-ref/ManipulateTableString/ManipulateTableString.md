...
|`Operator`|The operation to perform on the input strings.  See the table below for a summary of the input and output parameters required for each operator..|None – must be specified.|
...
|`InputValue2`|A string constant, as the second input.  Can be specified with `${Property}` notation.  Specific requirements for `Operator` are:<ul><li>`Remove` - string or expression in input to remove:<ul><li>combination of `^` to indicate start of line, `$` to indicate end of line, `\$` to match a dollar sign, `\s` to indicate a space, and character pattern to remove</li><li>`[ABC]` - a list of characters to remove from input</li></ul></li><li>`Replace` -  string or expression in input to remove<ul><li>combination of `^` to indicate start of line, `$` to indicate end of line, `\$` to match a dollar sign, `\s` to indicate a space, and character pattern to remove</li></li></ul></ul>|Required if a 2nd input value is needed and no `InputColumn2`.|
...
|`UseEmptyStringForNullInput` | If `True` set null input from table cell values to an empty string. | `False` |
|`OutputColumn`|The name of a column to receive the output.  The output column can be the same as one of the input columns to update a column. Can be specified with `${Property}` notation.|None – must be specified.|
...
|`Copy` | Copy the first input to the output (requires 1 input). |
...
|`ToBoolean` | Convert the first input to a boolean object. Input can be `true` or `false` (any case), or 0 or 1 (as string, integer, or long integer). |
...
|`ToLong` | Convert the first input to a long integer object.  If the input string contains a floating point number, the number will be rounded. |
