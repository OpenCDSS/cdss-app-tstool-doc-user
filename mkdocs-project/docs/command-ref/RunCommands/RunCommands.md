...
*   Commands that are in a main command file typically run in discovery mode when opened in the user interface,
    to ensure that information like time series identifiers can be passed to following commands for editing.
    However, because the commands that `RunCommands` is running cannot be directly edited,
    there is no need to run discovery mode.
...
The following dialog is used to edit the command and illustrates the input parameters syntax.

**<p style="text-align: center;">
![RunCommands command editor for input parameters](RunCommands-input.png)
</p>**

**<p style="text-align: center;">
`RunCommands` Command Editor for Input Parameters (<a href="../RunCommands-input.png">see full-size image</a>)
</p>**

### Output ###

The following dialog is used to edit the command and illustrates the output parameters syntax.

**<p style="text-align: center;">
![RunCommands command editor for output parameters](RunCommands-output.png)
</p>**

**<p style="text-align: center;">
`RunCommands` Command Editor for Output Parameters (<a href="../RunCommands-output.png">see full-size image</a>)
</p>**

### Shared Data ###

The following dialog is used to edit the command and illustrates the shared data parameters syntax.

**<p style="text-align: center;">
![RunCommands command editor for shared data parameters](RunCommands-shared-data.png)
</p>**

**<p style="text-align: center;">
`RunCommands` Command Editor for Shared Data Parameters (<a href="../RunCommands-shared-data.png">see full-size image</a>)
</p>**

### Test ###

The following dialog is used to edit the command and illustrates the test parameters syntax.

**<p style="text-align: center;">
![RunCommands command editor for shared data parameters](RunCommands-test.png)
</p>**

**<p style="text-align: center;">
`RunCommands` Command Editor for Test Parameters (<a href="../RunCommands-test.png">see full-size image</a>)
</p>**
...
| --| --------------|-----------------|----------------- |
| ***Input*** | `InputFile`<br>**required** | The command file to run. A path relative to the master command file can be specified.  Can use `${Property}`. | None - must be specified. |
| ***Output*** | `AppendOutputFiles` | Indicate whether output files generated by the command file that is run should be listed in the main TSTool ***Results / Output Files*** (`True`) or not (`False`).  The default is `False` because a large number of output files may be generated. | `False` |
...
| ***Shared Data*** | `ShareDatastores` | Indicate whether datastores in the parent should be shared with the child command processor.  Normally this should be done so that databases can be opened once.  Note that opening datastores in the child command file will not make the datastores available in the parent. | `Share` |
| | `IncludeProperties` | Specify a comma-separated list of properties to include, can use `*`. | None. |
| | `ExcludeProperties` | Specify a comma-separated list of properties to exclude, can use `*`. | None. |
| ***Test*** | `RunDiscovery` | Whether to run commands in `InputdFile` in discovery mode as they are loaded (`True`) or not (`False`).  This parameter is used by software developers. | `False` | 
