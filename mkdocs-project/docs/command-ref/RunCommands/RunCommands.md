...
| --------------|-----------------|----------------- |
| `InputFile`<br>**required** | The command file to run. A path relative to the master command file can be specified.  Can use `${Property}`. | None - must be specified. |
| `RunDiscovery` | Whether to run commands in `InputdFile` in discovery mode as they are loaded (`True`) or not (`False`).  This parameter is used by software developers. | `False` | 
