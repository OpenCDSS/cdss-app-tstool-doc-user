...
|***List***|`List` | A list of comma-separated values to be used as variables for the iteration. Can use `${Property}` syntax. As of TSTool 15.4.0 the items in the list are not trimmed for surrounding white space characters. | No default if list is used – must specify a list of values. |
